...
const validateCategory = (category) => {
    ...
    if (!category.name || /^\s*$/.test(category.name)) {
        validationErrors.name = 'Имя жанра обязательно и не должно начинаться с пробела.';
    } else if (!/^[a-zA-Zа-яА-ЯёЁ\s]+$/.test(category.name)) {
        validationErrors.name = 'Имя жанра может содержать только буквы.';
    }
...
const refreshData = async () => {
    try {
        const response = await fetch('/api/categorieslist');
...
const onRowEditSave = (event) => {
    ...
    const validationErrors = validateCategory(newData);
    if (Object.keys(validationErrors).length > 0) {
        errors.value = validationErrors;
        return;
    }

    Inertia.put(`/api/categorieslist/${newData.id}`, {
        name: newData.name,
    }).then(response => {
...
const addCategory = () => {
    ...
    Inertia.post('/api/categorieslist', categoryData, {
        onSuccess: (response) => {
            if (response.props && response.props.errors) {
                errors.value = response.props.errors;
            } else {
...
const deleteRow = (row) => {
    confirm.require({
        ...
        message: `Вы уверены, что хотите удалить жанр - ${row.name}?`,
        ...
        accept: () => {
            ...
            Inertia.delete(`/api/categorieslist/${row.id}`)
                .then(response => {
                    if (response.props && response.props.errors) {
                        errors.value = response.props.errors;
                    } else {
...
<template>
    <Head>
        <meta head-key="description" name="description" content="Ведение списка жанров">
    </Head>
    <h1 class="title">Ведение списка жанров</h1>
    ...
    <div class="card">
        ...
        <template #header>
            <div class="flex justify-between pb-4">
                <Button label="Добавить жанр" icon="pi pi-plus" @click="showDialog = true" />
        ...
        <Dialog header="Добавить жанр" v-model:visible="showDialog" :modal="true" :closable='false'>
            <InputText 
            ...
            <div class="flex justify-end">
                ...
                <Button label='Добавить' icon='pi pi-check' @click='addCategory' />
            </div>
